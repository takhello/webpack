...
const getArguments = (schema = webpackSchema) => {
	...
	// 解析 $ref
	// 参考："#/definitions/AssetGeneratorDataUrl" -> definitions.AssetGeneratorDataUrl
	 ...
	 */
	console.log("7.1.1 getArguments schema");
	...
	const traverse = (schemaPart, schemaPath = "", path = [], inArray = null) => {
		...
		//schema：definitions.path：Amd  path：Amd
			...
			if (schemaPart.properties) {
				for (const property of Object.keys(schemaPart.properties)) {
					addedArguments += traverse(
						schemaPart.properties[property], // 注意，这里获取schemaPart.properties下对应属性
	...
	// const node_util_1 = require("@tk-tool/node-util");
	// node_util_1.writeSync({
	// 	path: "temp/flags.json",
	// 	data: flags
	// });
	return flags;
};
